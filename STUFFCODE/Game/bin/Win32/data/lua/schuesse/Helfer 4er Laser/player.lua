...
function Shoot(c0,c1,c2,c3,c4,c5,PlayerNum,X,Y,Angle,Speed)
	...
	X = X -10; --64.0 + (60.0 * math.sin(Angle + math.pi/2)); -- Wo des Schuss auf der X Achse gespawned wird. X input ist die X coordinate des Spielers
	Y = Y - 40; --32.0 - (28.0 * math.cos(Angle + math.pi/2)); -- ^^ Y

	Cooldown = 4; -- Wie Lange bis zum nächsten Schuss. In 1/60 Sekunden gemessen
	...
	CreateBullet(PlayerNum, 1, X, Y, 0, Angle);	-- Spawned einen Schuss mit Angegbene Werten. Dieser Code kann auch mehrmals aufgerufen Werden
	CreateBullet(PlayerNum, 1, X, Y + 115, 0, Angle);
	
	CreateBullet(PlayerNum, 2, X - 10, Y, 10, 45);
	CreateBullet(PlayerNum, 2, X - 10, Y, 10, 90);
	CreateBullet(PlayerNum, 2, X - 10, Y, 10, 180);
	CreateBullet(PlayerNum, 2, X - 10, Y, 10, -45);
	
	CreateBullet(PlayerNum, 2, X - 10, Y + 115, 10, 45);
	CreateBullet(PlayerNum, 2, X - 10, Y + 115, 10, 90);
	CreateBullet(PlayerNum, 2, X - 10, Y + 115, 10, 180);
	CreateBullet(PlayerNum, 2, X - 10, Y + 115, 10, -45);
